...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section className="section capabilities"><div><p className="eyebrow">Capabilities</p><h2>Systems thinking<br />with a builder&apos;s <em>finish.</em></h2></div><ul>{capabilities.map((capability, index) => <li key={capability}><span>0{index + 1}</span>{capability}<ArrowUpRight size={19} /></li>)}</ul></section>

      <section className="section about-company" id="about"><div className="about-company__heading"><p className="eyebrow">About / Résumé</p><h2>One person,<br /><em>deep in the work.</em></h2></div><div className="about-company__copy"><p>Rylee Benson is the founder and full-stack engineer behind Ecosystem Global Solutions Inc. The practice is deliberately close to the work: systems, interfaces, and technical decisions are built together rather than handed between layers.</p><p>This portfolio is an evidence-led record of that work. Each case study separates what is currently verified from what is exploratory, and links out where there is a public surface to inspect.</p><a className="text-link" href="mailto:owner@outboundautonomy.com?subject=Portfolio%20enquiry">Start a conversation <span>↗</span></a></div></section>
